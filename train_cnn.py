...
#         # 读取表格
#         self.data = pd.read_csv(csv_file)  # 或 pd.read_csv(csv_file)
#         base_dir = "/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/"
...
#             print(f"Found {len(self.unique_labels)} classes: {self.class_to_idx}")
...
# trainset = ExcelImageDataset(csv_file='/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/2.1.0/train.csv', transform=train_transformer)

# testset = ExcelImageDataset(csv_file='/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/2.1.0/test.csv', transform=test_transformer)


# trainset = CSVImageDataset(csv_file="/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/2.1.0/train.csv", transform=train_transformer)
# testset = CSVImageDataset(csv_file="/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/2.1.0/test.csv", transform=test_transformer)

class CustomImageDataset(Dataset):
    def __init__(self, csv_file, root_dir="", transform=None, verbose=True):
        """
        csv_file: 包含图像路径和标签的 CSV 文件
        root_dir: 图像所在的根目录
        transform: 图像预处理
        verbose: 是否打印加载信息
        """
        ...
        for i in range(len(df)):
            rel_path = str(df.iloc[i, 0])
            label = df.iloc[i, 1]
            img_path = os.path.join(self.root_dir, rel_path)
            img_path = os.path.abspath(img_path)  # 转绝对路径
        ...
        if verbose:
            print(f"✅ Loaded {len(self.img_paths)} valid images out of {len(df)} rows.")
...
trainset = CustomImageDataset(
    csv_file="/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/2.1.0/train.csv",
    root_dir="/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/",
    transform=train_transformer
)
testset = CustomImageDataset(
    csv_file="/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/2.1.0/test.csv",
    root_dir="/Users/kindle/Desktop/file/project/LLM-with-mixed-type-data/data/raw/mimic-cxr-jpg/",
    transform=test_transformer
)
