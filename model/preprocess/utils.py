...
from constants import mimic_cxr_data_dir

reports_path = mimic_cxr_data_dir / "text_file"
print(reports_path)
...
def section_text(text):
    ...
    # s.start(1) return the first group's first item's position in txt
    if s:
        ...
        while s:
            # s.group(1) returns first group's ()
            ...
            # find returns the first item's position 
